        ...
        public void ConfigureServices(IServiceCollection serviceCollection)
        {
            ...
            serviceCollection.AddOpenApi("v1", options =>
            {
                ...
                options.AddDocumentTransformer((document, context, ct) =>
                {
                    ...
                    {
                        Title = "KSS.Service.Company",
                        Version = "v1"
                    };
                    // Set HTTPS server for production to avoid mixed content errors
                    // In dev, don't set servers - Scalar uses current origin (localhost/127.0.0.1)
                    var env = context.ApplicationServices.GetRequiredService<IWebHostEnvironment>();
                    if (!env.IsDevelopment())
                    {
                        document.Servers =
                        [
                            new OpenApiServer { Url = "https://main.api.sebaoffice.ir", Description = "Production" }
                        ];
                    }
